...
import { Check, Star, Zap, Tag, Layers, Info, Box, Slash } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          console.error("Failed to fetch product from API", await res.text());
          setProduct(null);
          return;
        }

        const json = await res.json();
        const data = json?.product ?? null;
        if (!data) {
          setProduct(null);
          return;
        }
        ...
        const normalized = {
          ...
          longDescription: data.product_description ?? "",
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="p-0 border border-border rounded-lg overflow-hidden h-full flex flex-col shadow-sm">
                  ...
                  <div className="p-4 border-t border-border text-xs text-muted-foreground flex items-center gap-2">
                    <Info size={14} />
                    Note: Specifications are provided by the manufacturer and
                    may vary.
                  </div>
                </div>
              </div>
